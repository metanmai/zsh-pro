...
func TestRegenRoundTrip(t *testing.T) {
	...
	src := []byte("export EDITOR=nvim\n" +
		...
		"alias -g GG='| grep'\n" +
		// Adversarial: single-line array assignment (UAT array gap) — must keep the
		// full `(...)` value, never regenerated as a bare `plugins=`.
		"plugins=(git zsh-autosuggestions zsh-syntax-highlighting)\n" +
		// Adversarial: multi-line array assignment (UAT array gap) — elements and
		// the `(...)` must survive the round-trip.
		"marr=(\n  a\n  b\n)\n")
	...
	// UAT array gap: a single-name array assignment must keep its FULL `(...)`
	// value — never be regenerated as a bare `plugins=` (the exact corruption the
	// real-config UAT caught: oh-my-zsh loaded zero plugins). The router routes it
	// imperative, so it round-trips as verbatim Text.
	if !bytes.Contains(regen, []byte("plugins=(git zsh-autosuggestions zsh-syntax-highlighting)")) {
		t.Errorf("array assignment dropped — regenerated as bare name= (UAT array gap); regen=%q", regen)
	}
	// UAT array gap (multi-line): the multi-line array's `(` and its elements must
	// survive the round-trip verbatim, not collapse to a bare `marr=`.
	if !bytes.Contains(regen, []byte("marr=(")) {
		t.Errorf("multi-line array opening dropped (UAT array gap); regen=%q", regen)
	}
	for _, elem := range []string{"a", "b"} {
		if !bytes.Contains(regen, []byte(elem)) {
			t.Errorf("multi-line array element %q absent from regen (UAT array gap); regen=%q", elem, regen)
		}
	}
}
