...
func TestRegenerateSourceOrderAndRouting(t *testing.T) {
	profile := model.Profile{Entries: []model.Entry{
		...
		{Text: "alias gs='git status'", Kind: model.KindAlias, Names: []string{"gs"}, Value: "'git status'", Managed: true},
...
func TestRegenerateDynamicValueDelegated(t *testing.T) {
	...
	profile := model.Profile{Entries: []model.Entry{
		{Text: "export GOPATH=$HOME/go", Kind: model.KindAssignment, Names: []string{"GOPATH"}, Value: "$HOME/go", Exported: true, Managed: true, Dynamic: true},
	}}
