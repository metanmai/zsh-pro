...
- ✓ **Git-backed, branchable profile store** — `core/store` initializes a bare git repo via the `git` binary (no new dependency; absent-`git` degrades with `ErrGitAbsent`, never panics), branch = profile; `New`/`Init`/`Branches`/`Current`/`Create`/`Checkout`/`Read`/`Commit` typed on `model.Profile` — `Commit` writes plumbing-only (no checkout / working-tree mutation), `Read` via `git show`; active profile tracked **per-terminal** via `ZSHPRO_PROFILE` (never a shared global file). `Read(Commit(p))` round-trip pinned and composed with the Phase 2 byte-identical oracle. Detected **literal** secrets are excluded from the committed tree and captured into an OS keychain / 0600 file vault (`SecretRef{kind:key}` + withheld report), **failing closed** (`ErrUnsafeSecretShape`) on unsafe multi-name/array shapes so no literal can leak (T-03-03); already-dynamic secrets commit verbatim. Durable outputs: `core/store`, `03-VERIFICATION.md` — v2.0 Phase 3 (PROF-01, PROF-02; PROF-03 started)
...
---
*Last updated: 2026-06-25 — pivoted to v2.0 (Branchable Shell Environments): corrected the project identity from "read-only analyzer" to a git-versioned, branchable shell-environment manager (the analyzer is now its ingest component). v1.1 parked partway (Phase 2 shipped) — see MILESTONES.md. · v2.0 Phase 1 (SPIKE) complete 2026-06-25 — zero-residue live hot-switch validated **GO**. · v2.0 Phase 2 (IR + Partial Evaluation) complete 2026-06-27 — `model.Profile`/`Entry` IR, declarative/imperative gate, static/dynamic tagging, byte-identical round-trip oracle (ING-01/ING-02/EVAL-01 validated; code review found+fixed a panic and a silent multi-name corruption before sign-off). · v2.0 Phase 3 (Git-Backed Store) complete 2026-06-27 — `core/store` git-binary-backed branchable profile store (Init/Branches/Current/Create/Checkout/Read/Commit on `model.Profile`, per-terminal `ZSHPRO_PROFILE`), `Read(Commit(p))` round-trip composed with the Phase 2 oracle, store-side literal-secret exclusion → keychain/0600-vault `SecretRef`. Code review found+fixed 2 secret-leak blockers (T-03-03 multi-name/array assignments now fail closed); verifier passed 11/11 (PROF-01, PROF-02 done; PROF-03 started). Next: Phase 4 (Manifest Builder + Emit).*
