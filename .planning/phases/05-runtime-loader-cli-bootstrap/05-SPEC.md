...
Phase 4 (Manifest Builder + Emit) produces the reversible record and the zsh apply/deactivate code: `core/activate` builds a `model.Manifest` and a shell-agnostic `Plan`, and `core/shell/zsh/emit.go` renders that plan to zsh code that calls a fixed set of `zp_*` runtime helpers (`zp_capture_env`/`zp_restore_env`/PATH-rebuild-from-base/shadow-capture/shadow-restore) — pinned to **bare calls** to those helpers (Phase 4 OQ-5/OQ-18, `emit.go` does not self-define them). Phase 4 explicitly deferred to Phase 5: (a) the sourced runtime loader itself, (b) the five CLI verbs, (c) per-terminal active-state wiring, (d) the `.zshrc` bootstrap block, and (e) **the base-capture placement** — which loader entry point captures `ZP_BASE_PATH` and the re-capture guard (Phase 4 OQ-3, "finalized in Phase 5's loader"). Phase 5 **defines the `zp_*` helper bodies** the emitted code calls (Phase 4 OQ-5 seam), and captures `ZP_BASE_PATH` **once at first activate** into per-terminal runtime state (Phase 4 OQ-3 mechanism locked; placement lands here).
...
1. **`hook` subcommand emits the embedded loader**: `zsh-pro hook` prints the sourced loader script to stdout.
   ...
   - Target: `zsh-pro hook` (mirroring the `introspectScript` `const` embed pattern in `core/shell/zsh`) writes a complete zsh loader to stdout for `eval "$(zsh-pro hook)"`. The loader defines the `zp_*` runtime helpers the Phase-4-emitted code calls (`zp_capture_env`, `zp_restore_env`, PATH-rebuild-from-base, shadow-capture/restore), the `ZP_UNSET_SENTINEL`, and the shell functions `checkout`/`activate`/`deactivate`/`list`/`status`. The emitted script is `zsh -n`-parseable.
   - Acceptance: `zsh-pro hook | zsh -n` exits 0 (syntax-valid); the output defines all five verb functions and the `zp_*` helper set (grep-verified); the loader script lives as an embedded constant in `core/shell/zsh` (grep-verified — not assembled in `core/cli`).
...
- The loader's definition of the `zp_*` runtime helper bodies the Phase-4-emitted code calls (the Phase 4 OQ-5 seam — Phase 4 emits bare calls; Phase 5 owns the definitions).
...
- **`zp_*` helper contract** — the loader's helper bodies must match the contract Phase 4's emitted code calls (bare `zp_*` calls, OQ-5/OQ-18) and must reproduce the Phase 1 loader-reference behavior WITH the OQ-8 corrections (`${+name}` set-tests for shadow restore, `${(P)+var}==1` for env unset-vs-empty), NOT the buggy `-n`/`${(P)+literalName}` guards from the raw Phase 1 snippet.
