...
- **D-01** Loader defines exactly the `zp_*` helper set Phase 4's `emit.go` emits bare calls to: `zp_capture_env <var>`, `zp_restore_env <var> <applied>`, PATH-rebuild-from-base, shadow-capture/restore for aliases+functions. Signatures locked from the Phase 1 reference snippet + Phase 4 D-12/OQ-8.
...
> **Plan-time gate (OQ-05-05):** Phase 4 is documented but **not yet on disk** — `core/shell/zsh/emit.go`, `core/activate`, `core/model/manifest.go` do not exist yet. Lock the contract below from Phase 1's reference snippet + Phase 4 D-12/OQ-8; at plan/execute time, **diff the loader's defined helpers against `emit.go`'s literal bare calls** and reconcile any signature mismatch (localized — both live in `core/shell/zsh`).

Derived from the Phase 1 Loader Reference Snippet (`01-FINDINGS.md` §Loader Reference Snippet) + Phase 4 D-12/OQ-8 + the validated manifest shape (`01-MANIFEST-SHAPE.md`):
