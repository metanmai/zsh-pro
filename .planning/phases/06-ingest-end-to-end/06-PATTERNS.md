...
- The private journal binds parent device/inode, journal descriptor identity/mode/uid/gid/digest/schema/transaction ID, requested-link topology, `exchangePeerBasename`, `candidateEvidenceBasename`, and `journalBasename`. It defines no additional displaced-occupant artifact. Every mutation and cleanup reauthenticates those fields.
- New bytes at exchangePeerBasename and independent identity/digest/mode evidence at candidateEvidenceBasename are fsynced and closed before the prepared journal is written/fsynced and before target `atomicRenameAt`; candidate/evidence failure advances no journal, performs no target namespace call, and leaves the target unchanged.
