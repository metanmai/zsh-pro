...
- New bytes at exchangePeerBasename and independent candidate identity/digest/mode evidence at candidateEvidenceBasename must each be fsynced and closed before the prepared journal is written/fsynced and before any target namespace syscall. Candidate/evidence failure advances no journal, performs no target namespace call, and leaves the target unchanged.
- Recovery authenticates parent device/inode, journal descriptor identity/mode/uid/gid/digest/schema/transaction ID, requested-link topology, exchangePeerBasename, candidateEvidenceBasename, and journalBasename before identifying pre/post/reversed rows. It does not infer success from journal state alone and defines no additional displaced-occupant artifact.
- The identity axes are independent: expectedTarget is the bounded postInstallSnapshot (existence, requested-link topology, regular-file type, device/inode, digest, and permission bits), while expectedCandidate comes from candidate evidence. Before exchange target/peer match expectedTarget/expectedCandidate; afterward target matches expectedCandidate and peer/displacedObserved is compared with expectedTarget. Target uid/gid, timestamps, ACLs, xattrs, and platform flags are excluded; journal ownership is another authentication axis.
...
- Separate typed update-ref encoders are selected only after RefPresent/ExpectedRevision validation: absent emits the complete create transaction and present emits the complete update(expected) transaction. Callers cannot provide raw verbs, and invalid state starts no ref process.
...
- Valid persisted SecretRefs are proven at CommitIngest level with a fixed call/effect matrix: structural/unsupported-kind rejection has Kind=0; structurally valid wrong-backend-kind and valid pass-through have Kind=1; every row has Retrieve/Store/Delete=0. Counters reset after Begin prove malformed rows perform zero candidate object writes, final publication, ref-process starts, and ref changes; valid pass-through remains exact with no second withheld record.
...
- EffectiveManaged is the sole partition predicate. The master subtracts only exact byte-addressed extents represented by EffectiveManaged entries in the resulting committed Profile; it does not positive-select recognized imperative kinds or reconstruct from `Entry.Text`. Imperative, parseable per-statement opaque, forced-unmanaged, comment, and gap bytes survive, and ambiguous/unmapped association fails before commit. The unit and authored E2E fixtures both contain imperative plus `export $FLAGS FOO=1` opaque statements in source order.
- Every failure after initial promotion and before Commit uses one `compensatePreCommitFilesystemFirst` path. Its exact order is filesystem classify/restore-or-unchanged -> AbortIngest -> loader/cache rollback -> initializer rollback. Only unchanged/restored-durable advances; recovery-required, sync-uncertain, unsafe-reverse, or absent-post-create evidence retains target/journal/Store quarantine/cache/initializer and keeps later counters zero. `ir.Build` is total, so the real injected seams are parse, duplicate/extent association, accounting, candidate preparation, and expectedTarget validation.
...
| `core/store/install_transaction_darwin_test.go` | 06-01 | native production-helper cleanup tests for top-level, nested file/directory, symlink, and both replacement seams |
...
| `06-MACOS-RUNTIME-EVIDENCE.md` | 06-05 | attributable native macOS toolchain/GOVERSION/platform/filesystem/command/output atomic and six-row cleanup PASS record |
...
6. Run common/per-platform atomic selectors, `TestAtomicRenameLinuxGo125ABIAndFallbackSurface`, per-call parallel counters, direct adapter identity/no-replace/unsupported-before-effects tests, candidate/evidence-fsync-before-journal failure, target-root lock tests, static mutation-surface audit, changed parent/journal/link authentication, and eight separately named existing/absent crash rows; absent post-create rollback remains recovery required.
...
8. Cross-compile Linux and Darwin amd64/arm64 with CGO disabled under local Go 1.25.x, then block phase completion on attributable native macOS atomic plus production Store cleanup evidence for the exact implementation SHA. Require PASS rows for top-level, nested file/directory, symlink, and both replacement seams; an unsupported cleanup primitive qualifies only as untouched retained/recovery behavior. Record `go_toolchain: local` and exact `go_env_goversion`.
